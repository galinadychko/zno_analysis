...
test_that("Test GeneralisedNadarayaWatson/train(not_correct_input)", {
  ...
  w_coeff <- c(NaN)
  instance <- GeneralisedNadarayaWatson$new()
  expect_error(instance$train(X_train, Y_train, w_coeff), "Not correct class attributes")
  expect_error(instance$train(as.numeric(X_train), Y_train, w_coeff), "Not correct class attributes")
  expect_error(instance$train(X_train, as.numeric(Y_train), w_coeff), "Not correct class attributes")
  expect_error(instance$train(X_train, Y_train, as.matrix(w_coeff)), "Not correct class attributes")
...
test_that("Test GeneralisedNadarayaWatson/predict(not_correct_input)", {
  instance <- GeneralisedNadarayaWatson$new()
  expect_error(instance$predict(rep(NaN), rbind(NaN), 1), "The model was not trained correctly")
  instance <- GeneralisedNadarayaWatson$new()
  instance$X_train <- 1:2
  instance$Y_train <- 1:2
  instance$A <- matrix(NaN, 2, 2)
  expect_error(instance$predict(rep(NaN, 1), rbind(NaN), 1), "The model coefficients are not numbers")
  instance$A <- matrix(1, 2, 2)
  expect_error(instance$predict(rep(NaN, 1), matrix(NaN, 2, 2), 1), "Not correct input dimensions")
})
...
test_that("Test GeneralisedNadarayaWatson/predict(correct_input)", {
  ...
  instance <- GeneralisedNadarayaWatson$new()
  instance$train(X_train, Y_train, w_coeff)
  expect_equal(instance$predict(X_test, w_coeff, 1),
  ...
  expect_equal(instance$predict(c(X_test2, 1), w_coeff2, 1),
...
test_that("Test output type GeneralisedNadarayaWatson/predict", {
  ...
  instance <- GeneralisedNadarayaWatson$new()
  instance$train(X_train, Y_train, w_coeff)
  expect_true(is.list(instance$predict(X_test, w_coeff, 1)))
  expect_true(is.matrix(instance$predict(X_test, w_coeff, 1)[[1]]))
  expect_true(is.matrix(instance$predict(X_test, w_coeff, 1)[[2]]))
  expect_true(is.matrix(instance$predict(X_test, w_coeff, 0.5)[[1]]))
  expect_true(is.matrix(instance$predict(X_test, w_coeff, 0.5)[[2]]))
  instance <- GeneralisedNadarayaWatson$new()
  instance$train(c(X_train, 3), c(Y_train, 1), w_coeff2)
  expect_true(is.list(instance$predict(X_test2, w_coeff, 1)))
  expect_true(is.matrix(instance$predict(c(X_test2, 1), w_coeff2, 1)[[1]]))
  expect_true(is.matrix(instance$predict(c(X_test2, 1), w_coeff2, 1)[[2]]))
})
...
test_that("Test GeneralisedNadarayaWatson/predict_in_parallel(not_correct_input)", {
  ...
  expect_error(instance$predict_in_parallel(rep(NaN), cbind(NaN), 1), 
  ...
  instance$A <- matrix(NaN, 2, 2)
  instance$run_cluster(1)
  expect_error(instance$predict_in_parallel(rep(NaN, 1), cbind(NaN, 1), 1), 
               "The model coefficients are not numbers")
  instance$stop_cluster()

  instance$run_cluster()
  expect_error(instance$predict_in_parallel(rep(NaN, max_threads), matrix(NaN, max_threads, 2), h = 1), 
               "The model coefficients are not numbers")
  instance$stop_cluster()
  
  instance$A <- matrix(1, 2, 2)
  instance$run_cluster()
  expect_error(instance$predict_in_parallel(rep(NaN, 1), matrix(NaN, 2, 2), 1), 
...
test_that("Test GeneralisedNadarayaWatson/predict_in_parallel(correct_input)", {
  ...
  expect_equal(instance$predict_in_parallel(X_test, w_coeff, 1),
               list("prediction" = rbind(c(2, 2), c(2, 2)),
                    "A_test" = cbind(c(0, 2), c(4, -2))))
  expect_equal(instance$predict_in_parallel(X_test, w_coeff, .5),
               list("prediction" = rbind(c(2, 2), c(2, 2)),
                    "A_test" = cbind(c(0, 2), c(4, -2))))
  instance$stop_cluster()
  ...
  instance$train(c(X_train, 3), c(Y_train, 1), w_coeff2)
  instance$run_cluster(2)
  expect_equal(instance$predict_in_parallel(c(X_test2, 1), w_coeff2, 1), 
...
test_that("Test output type GeneralisedNadarayaWatson/predict_in_parallel", {
  ...
  expect_true(is.matrix(instance$predict_in_parallel(X_test, w_coeff, .5)[[2]]))
  instance$stop_cluster()

  instance <- GeneralisedNadarayaWatson$new()
  instance$train(c(X_train, 3), c(Y_train, 1), w_coeff2)
  instance$run_cluster(2)
  expect_true(is.list(instance$predict_in_parallel(c(X_test2, 1), w_coeff2, 1)))
  expect_true(is.matrix(instance$predict_in_parallel(c(X_test2, 1), w_coeff2, 1)[[1]]))
  expect_true(is.matrix(instance$predict_in_parallel(c(X_test2, 1), w_coeff2, 1)[[2]]))
  instance$stop_cluster()
})
