...
test_that("Test Epanechnikov(not_correct_input_type)",{
  ...
  expect_error(Epanechnikov(as.data.frame(M)), "Not correct input type")
  expect_error(Epanechnikov(as.list(M)), "Not correct input type")
})
...
test_that("Test nw_any_components(not_correct_input_type)",{
  ...
  expect_error(nw_any_components(2, rep(2, 2), rep(2, 2), 1, matrix(0, 2, 2)), "Devision by zero")
})
...
test_that("Test nw_any_components(different_values)",{
  ...
  expect_equal(nw_any_components(x, X_train, Y_train, c(1, 2), matrix(1:4, 2, 2)), c(2, 2))
})
...
test_that("Test output type nw_any_components",{
  ...
  expect_true(is.vector(nw_any_components(x, X_train, Y_train, c(1, .9), matrix(1:4, 2, 2))))
})
