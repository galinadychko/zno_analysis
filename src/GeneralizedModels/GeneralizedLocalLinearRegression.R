...
GeneralisedLocalLinearRegression <- R6Class("GeneralisedLocalLinearRegression", 
                                            public = list(
                                              X_train = NULL,
                                              Y_train = NULL,
                                              A = NULL,
                                              ...
                                              train = function(X_train, Y_train, W_train, h){
                                                ...
                                                self$A <- as.matrix(acoeff(W_train))
                                                self$h <- h
                                              }, 
                                              predict = function(X_test, W_test, comp_number=NULL){
                                                ...
                                                results <- lapply(X_test, function(x){
                                                  A <-  split(self$A, rep(1:ncol(self$A), each = nrow(self$A)))
                                                  b_coeff <- lapply(A,
                                                                   function(a, x){
                                                                     n <- length(self$X_train)
                                                                     X <- as.matrix(self$X_train); Y <- as.matrix(self$Y_train)
                                                                     K <-  Epanechnikov((x - self$X_train)/self$h) 
                                                                     X <- cbind(rep(1, n), X)
                                                                     A <- rbind(a, a)
                                                                     K <-  rbind(K, K)
                                                                     b <- (((solve((t(X) * A * K) %*% X)) %*% t(X)) * A * K) %*% Y
                                                                     names(b) <- c("b0", "b1")
                                                                     return(b)
                                                                   }, x = x)
                                                  b_coeff <- do.call(cbind, b_coeff)
                                                  results <- matrix(c(1, x), nrow=1, ncol=2) %*% b_coeff[, ncomp]
                                                })
